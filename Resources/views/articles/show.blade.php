...
@section('content')
                ...
                <div class="px-4 lg:px-0 lg:mx-48">
                    <div class="flex items-center justify-between pt-6 mb-28">
                        <a href="{{-- route('articles') --}}"
                            ...
                            <x-svg icon="s-arrow-left" class="w-4 h-4 fill-current" />
    ...
    @can(App\Policies\ArticlePolicy::DISAPPROVE, $article)
        @if ($article->isPublished())
            <x-modal identifier="unpublishArticle" :action="route('admin.articles.disapprove', $article->slug())"
                title="Unpublish article" type="update">
    ...
    @can(App\Policies\ArticlePolicy::DECLINE, $article)
        @if ($article->isNotDeclined())
            <x-modal identifier="declineArticle" :action="route('admin.articles.decline', $article->slug())"
                title="Decline article" type="update">
    ...
    @can(App\Policies\ArticlePolicy::PINNED, $article)
        <x-modal identifier="togglePinnedStatus" :action="route('admin.articles.pinned', $article->slug())"
            :title="$article->isPinned() ? 'Unpin article' : 'Pin article'" type="update">
            @if ($article->isPinned())
