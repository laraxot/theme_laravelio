<div class="flex flex-col mb-4 w-full @if (isset($centered) && $centered) items-center @endif">
    ...
    @can(App\Policies\UserPolicy::BAN, $user)
        @if ($user->isBanned())
            <button type="button" class="button button-primary w-full mb-4" @click.prevent="activeModal = 'unbanUser'">Unban
                User</button>
        @else
            <button type="button" class="button button-danger w-full mb-4" @click.prevent="activeModal = 'banUser'">Ban
                User</button>
        @endif
    @endcan
    ...
    @if (Auth::check() && Auth::user()->isAdmin())
        @can(App\Policies\UserPolicy::DELETE, $user)
            <button type="button" class="button button-danger w-full mb-4" @click.prevent="activeModal = 'deleteUser'">Delete
                User</button>
        @endcan
    @endif
