    ...
    <div class="flex flex-col items-center">
        ...
        <div class="flex flex-col items-center gap-y-5 mt-10">
            ...
            <a class="text-gray-300 hover:text-twitter" target="_blank" rel="noopener" aria-label="Share on Twitter"
                href="http://twitter.com/share?text={{ urlencode('"' . $article->title() . '" by ' . ($article->author->twitter() ? '@' . $article->author->twitter() : $article->author->name()) . ' - ') }}&url={{ urlencode(Panel::get($article)->url()) }}">
            ...
            <a class="text-gray-300 hover:text-facebook" target="_blank" rel="noopener" aria-label="Share on Facebook"
                href="https://www.facebook.com/sharer/sharer.php?u={{ urlencode(Panel::get($article)->url()) }}&quote={{ urlencode('"' . $article->title() . '" by ' . $article->author->name() . ' - ') }}">
            ...
            <a class="text-gray-300 hover:text-linkedin" target="_blank" rel="noopener" aria-label="Share on LinkedIn"
                href="https://www.linkedin.com/shareArticle?mini=true&url={{ urlencode(Panel::get($article)->url()) }}&title={{ urlencode('"' . $article->title() . '" by ' . $article->author->name() . ' - ') }}">
