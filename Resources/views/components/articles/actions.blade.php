<div class="flex flex-col gap-x-4 gap-y-3 lg:flex-row lg:justify-between">
    <div class="flex flex-col gap-x-4 gap-y-3 lg:flex-row">
        @if (Auth::check() && $article->isAuthoredBy(Auth::user()))
            <x-theme::buttons.secondary-button href="{{ route('articles.edit', $article->slug()) }}"
                class="w-full">
                <span class="flex items-center gap-x-2">
                    <x-theme::svg icon="o-pencil" class="w-5 h-5" title="Edit" />
                    Edit article
                </span>
    ...
    @can(App\Policies\ArticlePolicy::DELETE, $article)
        <x-theme::buttons.danger-button tag="button" @click.prevent="activeModal = 'deleteArticle'" class="w-full">
            <span class="flex items-center gap-x-2">
                <x-theme::svg icon="o-trash" class="w-5 h-5" title="Delete" />
                Delete article
            </span>
